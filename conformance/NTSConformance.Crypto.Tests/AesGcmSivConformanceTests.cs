...
/// <summary>
/// RFC 8452 Appendix C.1: every published AEAD_AES_128_GCM_SIV vector, sealed and opened.
///
/// <para>
/// Norn's AES-GCM-SIV is BouncyCastle's <c>GcmSivBlockCipher</c>, so what is under test here is
/// not the mathematics — it is the wiring. Which of the two key lengths selects which AEAD, the
/// tag length, the byte order of the nonce, whether the tag comes before or after the
/// ciphertext, and whether associated data reaches the primitive at all. Every one of those can
/// be wrong while the round trip still works perfectly against itself, and every one of them
/// makes the result unreadable to chrony.
/// </para>
/// <para>
/// So the vectors are the point. A round trip proves Norn agrees with Norn; these prove it
/// agrees with the document, which is the only thing another implementation also read.
/// </para>
/// <para>
/// Twenty-four cases, taken verbatim from the RFC: empty plaintext, empty associated data,
/// lengths on and off the block boundary, and the long ones at the end where a mistake in how
/// POLYVAL is fed the length block shows up and nowhere earlier.
/// </para>
/// </summary>
[TestFixture]
public class AesGcmSivConformanceTests
{

    #region RFC 8452 Appendix C.1

    /// <summary>
    /// Sealing the plaintext produces exactly the octets the RFC prints.
    /// </summary>
    [TestCase("01000000000000000000000000000000",
    ...
    [TestCase("2d4ed87da44102952ef94b02b805249b",
    ...
    [TestCase("bde3b2f204d1e9f8b06bc47f9745b3d1",
    ...
    [TestCase("f901cfe8a69615a93fdf7a98cad48179",
    ...
    [Test]
    public void RFC8452_C1_Encrypt(String Key, String Nonce, String AAD, String Plaintext, String Expected)
    {

        var result = new AES_GCM_SIV(Bytes.FromHex(Key)).
                         Encrypt(Bytes.FromHex(AAD),
                                 Bytes.FromHex(Nonce),
                                 Bytes.FromHex(Plaintext));

        Assert.That(Bytes.ToHex(result),
                    Is.EqualTo(Expected),
                    $"key {Key}, nonce {Nonce}, AAD {AAD.Length / 2} bytes, " +
                    $"plaintext {Plaintext.Length / 2} bytes");
    ...
    #region And back again

    /// <summary>
    /// Opening the RFC's own ciphertext returns the RFC's own plaintext.
    /// </summary>
    /// <remarks>
    /// Not implied by the test above. An implementation that sealed correctly and opened
    /// incorrectly would pass every encryption vector and reject every real packet — and a
    /// decrypt tested only against a ciphertext this same code produced would never notice.
    /// </remarks>
    [TestCase("01000000000000000000000000000000",
              "030000000000000000000000",
              "",
    ...
    [TestCase("f901cfe8a69615a93fdf7a98cad48179",
              ...
              "391cc328d484a4f46406181bcd62efd9b3ee197d052d15506c84a9edd65e13e9d24a2a6e70")]
    [Test]
    public void RFC8452_C1_Decrypt(String Key, String Nonce, String AAD, String Plaintext, String Sealed_)
    {

        var result = new AES_GCM_SIV(Bytes.FromHex(Key)).
                         Decrypt(Bytes.FromHex(AAD),
                                 Bytes.FromHex(Nonce),
                                 Bytes.FromHex(Sealed_));
